...
final class FeaturePairTests: XCTestCase {
    ...
    private func assertNoRoundTripLoss(_ data: Data, file: StaticString = #filePath, line: UInt = #line) throws {
    ...
    func testMarkersFeaturePairRoundTripsAndMutates() throws {
        ...
        let marker = try XCTUnwrap(document.library?.events?.first?.projects?.first?.sequence?.spine?.assetClips?.first?.markers?.first)
        XCTAssertEqual(marker.start, "5s")
        XCTAssertEqual(marker.value, "Cue")

        document.library?.events?[0].projects?[0].sequence?.spine?.assetClips?[0].markers?[0].value = "Cue Renamed"
        let encoded = try FCPXMLParser().encode(document)
        let decoded = try FCPXMLParser().parse(data: encoded)
        XCTAssertEqual(
            decoded.library?.events?.first?.projects?.first?.sequence?.spine?.assetClips?.first?.markers?.first?.value,
            "Cue Renamed"
        )
        XCTAssertEqual(
            decoded.library?.events?.first?.projects?.first?.sequence?.spine?.assetClips?.first?.markers?.first?.start,
            "5s"
        )
    ...
    func testRolesFeaturePairRoundTripsAndMutates() throws {
        ...
        let clip = try XCTUnwrap(document.library?.events?.first?.projects?.first?.sequence?.spine?.assetClips?.first)
        ...
        document.library?.events?[0].projects?[0].sequence?.spine?.assetClips?[0].audioChannelSource?[0].role = "music.music-2"
        let encoded = try FCPXMLParser().encode(document)
        let decoded = try FCPXMLParser().parse(data: encoded)
        XCTAssertEqual(
            decoded.library?.events?.first?.projects?.first?.sequence?.spine?.assetClips?.first?.audioChannelSource?.first?.role,
            "music.music-2"
        )
        XCTAssertEqual(
            decoded.library?.events?.first?.projects?.first?.sequence?.spine?.assetClips?.first?.audioRole,
            "dialogue"
        )
        try assertNoRoundTripLoss(encoded)
    }
    ...
    func testTitlesFeaturePairRoundTripsAndMutates() throws {
        ...
        let title = try XCTUnwrap(document.library?.events?.first?.projects?.first?.sequence?.spine?.assetClips?.first?.titles?.first)
        ...
        document.library?.events?[0].projects?[0].sequence?.spine?.assetClips?[0].titles?[0].textStyleDef?[0].textStyle?.fontSize = "72"
        let encoded = try FCPXMLParser().encode(document)
        let decoded = try FCPXMLParser().parse(data: encoded)
        XCTAssertEqual(
            decoded.library?.events?.first?.projects?.first?.sequence?.spine?.assetClips?.first?.titles?.first?.textStyleDef?.first?.textStyle?.fontSize,
            "72"
        )
        XCTAssertEqual(
            decoded.library?.events?.first?.projects?.first?.sequence?.spine?.assetClips?.first?.titles?.first?.name,
            "Basic Title"
        )
        try assertNoRoundTripLoss(encoded)
    }
    ...
    func testRetimingFeaturePairRoundTripsAndMutates() throws {
        ...
        document.library?.events?[0].projects?[0].sequence?.spine?.assetClips?[0].timeMap?.timepts?[1].interp = "linear"
        let encoded = try FCPXMLParser().encode(document)
        let decoded = try FCPXMLParser().parse(data: encoded)
        XCTAssertEqual(
            decoded.library?.events?.first?.projects?.first?.sequence?.spine?.assetClips?.first?.timeMap?.timepts?[1].interp,
            "linear"
        )
        XCTAssertEqual(
            decoded.library?.events?.first?.projects?.first?.sequence?.spine?.assetClips?.first?.timeMap?.timepts?[1].value,
            "10s"
        )
        try assertNoRoundTripLoss(encoded)
    }
