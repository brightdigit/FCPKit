    ...
    public static func defaultSearchRoots() -> [URL] {
        ...
        return names.map {
            applications
                ...
                .appendingPathComponent("Contents/Frameworks/Interchange.framework/Versions/A/Resources", isDirectory: true)
        }
    }
...
public struct FCPXMLDTDValidator: Sendable {
    ...
    private func declaredVersion(in data: Data) throws -> String? {
        ...
        guard let regex = try? NSRegularExpression(pattern: #"<fcpxml\s+[^>]*version=\"([^\"]+)\""#) else {
            return nil
        }
    ...
    private func extractPath(from line: String) -> String? {
        ...
        if let range = line.range(of: #"No declaration for element\s+([A-Za-z0-9:_-]+)"#, options: .regularExpression) {
            let token = line[range]
                .split(whereSeparator: \.isWhitespace)
                .last
                .map(String.init)
